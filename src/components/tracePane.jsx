...
// import { X, FileText, ZoomIn, ZoomOut, Maximize2, Minimize2, Shrink } from "lucide-react";
...
//           x: 400,
//           y: 300,
//           data: { id: document_name, title: document_name }
//         }]);
//       } catch (err) {
...
//   // Fetch connected documents when a node is clicked
...
//         return [
//           { "relatedEntityId": `${nodeId}-conn-1`, "name": "AMENDS", "direction": "INCOMING", "document_number": `Mock Doc A` },
//           { "relatedEntityId": `${nodeId}-conn-2`, "name": "REFERENCES", "direction": "OUTGOING", "document_number": `Mock Doc B` }
//         ];
//       }
...
//       return [
//         { "relatedEntityId": `${nodeId}-conn-1`, "name": "AMENDS", "direction": "INCOMING", "document_number": `Mock Doc C` },
//         { "relatedEntityId": `${nodeId}-conn-2`, "name": "REFERENCES", "direction": "OUTGOING", "document_number": `Mock Doc D` }
//       ];
//     }
//   };
...
//       const connectedIds = edges.filter(e => e.from === nodeId).map(e => e.to);
      
//       setNodes(prev => prev.filter(n => !connectedIds.includes(n.id)));
...
//       const newNodes = connectedDocs.map((doc, index) => {
//         const angle = (index * (360 / connectedDocs.length) * Math.PI) / 180;
//         const radius = 200;
//         const displayTitle = doc.document_number || doc.relatedEntityId;
//         return {
//           id: doc.relatedEntityId,
//           x: clickedNode.x + radius * Math.cos(angle),
//           y: clickedNode.y + radius * Math.sin(angle),
//           data: { id: doc.relatedEntityId, title: displayTitle }
//         };
//       });

//       const newEdges = connectedDocs.map(doc => ({
//         from: nodeId,
//         to: doc.relatedEntityId
//       }));
...
//   return (
//     <>
//       <div className={`fixed ${isFullscreen ? 'inset-0' : 'right-0 top-0 h-full w-full sm:w-2/3'} bg-white shadow-2xl z-50 animate-slideIn flex flex-col`}>
...
//                       className={`bg-white rounded-lg border-2 p-3 shadow-lg cursor-pointer transition-all hover:shadow-xl hover:scale-105 ${
//                         node.id === node.data.id ? 'border-cyan-500' : 'border-gray-300'
//                       } ${expandedNodes.has(node.id) ? 'ring-2 ring-blue-300' : ''}`}
//                     >
//                       <div className="flex items-start gap-2">
//                         <FileText className={`w-5 h-5 flex-shrink-0 ${
//                           node.id === node.data.id ? 'text-cyan-500' : 'text-gray-500'
//                         }`} />
...
//         {/* Footer Info */}
//         <div className="bg-gray-50 border-t border-gray-200 p-3 text-sm font-thin text-gray-600">
//           <span className="font-light">{nodes.length}</span> document{nodes.length !== 1 ? 's' : ''} • 
//           <span className="font-light ml-1">{edges.length}</span> connection{edges.length !== 1 ? 's' : ''}
//         </div>
//       </div>
